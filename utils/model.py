...
def freeze_backbone(trainable_parameters_list,model):
    for name,param in model.named_parameters():
        if name.split(".")[0] not in trainable_parameters_list:
            print(name)
...
def get_model(args, configs, device, train=False):
    ...
    if train:
        if train_config["freeze_backbone"]:
            model.train()
            trainable_parameters_list = model_config["trainable_parameters"]["module_list"]
            model = freeze_backbone(trainable_parameters_list,model)
            scheduled_optim = ReduceLRPlateau(
