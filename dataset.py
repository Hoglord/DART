...
class Dataset(Dataset):
    def __init__(
        ...
        self.accent_to_select = preprocess_config['accents']
        ...
        with open(os.path.join(self.preprocessed_path, "accents.json")) as f:
            self.accent_map = json.load(f)
            
        self.accents_to_indices = dict()
        
        for _idx, acc in enumerate(preprocess_config['accents']):
            self.accents_to_indices[acc] = _idx
            
        self.basename, self.speaker, self.text, self.raw_text = self.process_meta(
            filename,
            self.accent_map,
            self.accent_to_select
        )
    ...
    def __getitem__(self, idx):
        ...
        accent = self.accent_map[speaker]        
        ...
        #     "{}.wav".format(basename)
        # )
        # hubert_output_path = os.path.join(
        #     self.preprocessed_path,
        #     "hubert_output",
        #     "{}-hubert-{}.npy".format(speaker, basename),
        # )
        # hidden_states = np.load(hubert_output_path)
        # raw_wav = librosa.load(wav_path,sr=16000,mono=True)[0]
        ...
        if accent in self.accent_to_select:
            sample = {
                "id": basename,
                "speaker": speaker_id,
                "text": phone,
                "raw_text": raw_text,
                "accents": self.accents_to_indices[accent],
                "mel": mel,
                "pitch": pitch,
                "f0": f0,
                "uv": uv,
    ...
    def process_meta(self, filename,accent_map,accent_to_select):
        with open(
            ...
            for line in f.readlines():
                n, s, t, r = line.strip("\n").split("|")
                if accent_map[s] in accent_to_select:
                    name.append(n)
                    speaker.append(s)
                    text.append(t)
                    raw_text.append(r)
    ...
    def reprocess(self, data, idxs):
        ...
        # hidden_states = [data[idx]["hidden_states"].squeeze(0) for idx in idxs]
        ...
        spker_embeds = np.concatenate(np.array([data[idx]["spker_embed"] for idx in idxs]), axis=0) \
            if self.load_spker_embed else None
        accents = [data[idx]["accents"] for idx in idxs]
        ...
        texts = pad_1D(texts)
        # hidden_states = pad_2D(hidden_states)
        mels = pad_2D(mels)
...
class TextDataset(Dataset):
    def __init__(self, filename, preprocess_config, model_config):
        self.cleaners = preprocess_config["preprocessing"]["text"]["text_cleaners"]
        self.preprocessed_path = preprocess_config["path"]["preprocessed_path"]
        self.load_spker_embed = model_config["multi_speaker"] \
            and preprocess_config["preprocessing"]["speaker_embedder"] != 'none'
    ...
    def __getitem__(self, idx):
        ...
        return (basename, speaker_id, phone, raw_text, spker_embed,self.accents_to_indices[accent])
    ...
    def collate_fn(self, data):
        ...
        spker_embeds = np.concatenate(np.array([d[4] for d in data]), axis=0) \
            if self.load_spker_embed else None
        accents = np.array([d[5] for d in data])
        # hidden_states = [d[5].squeeze(0) for d in data]

        texts = pad_1D(texts)
        # hidden_states = pad_2D(hidden_states)

        return ids, raw_texts, speakers, texts, text_lens, max(text_lens), spker_embeds, accents
