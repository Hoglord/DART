...
class ReferenceEncoder(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config):
        ...
        if model_config["prosody_modeling"]["model_type"] != "none":
            ...
            ref_enc_pad = model_config["reference_encoder"]["ref_enc_pad"]
...
class VarianceAdaptor(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config, train_config, d_model):
        ...
        # self.use_mha = model_config["HuBERT"]["use_multi_head_attention"]
        # self.num_heads = model_config["HuBERT"]["num_heads"]
    ...
    def forward(
        ...
        # if speaker_embedding is not None and not self.use_mha:
        #     x = x + speaker_embedding.unsqueeze(1).expand(
        #         -1, text.shape[1], -1
        #     )
        # else:
        #     x, attn_output_weights = self.multihead_attn(x, speaker_embedding, speaker_embedding)
...
class MLVAEencoder(nn.Module):
    ...
    def forward(self, mels, acc_labels=None):
        _,x = self.encoder(mels)
        output = self.mlvae(x, acc_labels)
        return output

    def inference(self, mels, acc_labels=None):
        _,x = self.encoder(mels)
        output = self.mlvae.inference(x, acc_labels)
        return output
